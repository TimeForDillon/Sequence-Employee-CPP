...
void personnelTest()
{
    ...
    cout << endl << "Testing the Personnel Class..." << endl << endl;
    for(int i = 0; i < 3; i++)
    {
        cout << "Enter Employee Name... " << i + 1 << ": ";
        cin >> name;
        cout << "Enter Employee ID Number... " << i + 1 << ": ";
        cin >> id;
        cout << "Enter Employee Wage... " << i + 1 << ": ";
        cin >> wage;
        group.employeeList[i] = Employee(name, id, wage);
        group.employeeList[i].calculateSalary();
        cout << endl;
    }

    double averageSalary = group.getAverageSalary();
    double averageWage = group.getAverageWage();

    cout << "Average Salary: " << averageSalary << endl;
    cout << "Average Wages: " << averageWage << endl;
}
